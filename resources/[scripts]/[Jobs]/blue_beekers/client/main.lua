...
function StopNPCJob(cancel)
	...
	if cancel then
		...
		--TriggerServerEvent('beekersjob:onNPCJobCompleted')
	end
end
			...
			if Config.EnableSocietyOwnedVehicles then
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_vehicle', {
					...
					if Config.MaxInService == -1 then
						...
						ESX.TriggerServerCallback('service:enableService', function(canTakeService, maxInService, inServiceCount)
	...
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'beekers_harvest', {
			...
			if data.current.value == 'gaz_bottle' then
				TriggerServerEvent('beekersjob:startHarvest')
			elseif data.current.value == 'fix_tool' then
				TriggerServerEvent('beekersjob:startHarvest2')
			elseif data.current.value == 'caro_tool' then
				TriggerServerEvent('beekersjob:startHarvest3')
			end
		end, function(data, menu)
			menu.close()
	...
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'beekers_craft', {
			...
			if data.current.value == 'blow_pipe' then
				TriggerServerEvent('beekersjob:startCraft')
			elseif data.current.value == 'fix_kit' then
				TriggerServerEvent('beekersjob:startCraft2')
			elseif data.current.value == 'caro_kit' then
				TriggerServerEvent('beekersjob:startCraft3')
			end
		end, function(data, menu)
			menu.close()
...
function OpenMobilebeekersActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_beekers_actions', {
		...
		elements = {
			...
			{label = 'Beekers Ouvert',     value = 'beekers_ouvert'},
			{label = 'Beekers Fermé',     value = 'beekers_ferme'},
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil or amount < 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						menu.close()
						TriggerServerEvent('billing:sendBill', GetPlayerServerId(closestPlayer), 'society_beekers', _U('beekers'), amount)
					end
				end
			end, function(data, menu)
				menu.close()
		...
		elseif data.current.value == 'beekers_ouvert' then
			TriggerServerEvent('AnnouncebeekersOuvert')
		elseif data.current.value == 'beekers_ferme' then
			TriggerServerEvent('AnnouncebeekersFerme')
					...
					if NPCOnJob then
						if NPCTargetDeleterZone then
							...
							if CurrentlyTowedVehicle == NPCTargetTowable then
								...
								TriggerServerEvent('beekersjob:onNPCJobMissionCompleted')
								StopNPCJob()
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('beekersjob:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('beekersjob:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('beekersjob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('beekersjob:putStockItems', itemName, count)
...
RegisterNetEvent('beekersjob:onHijack')
AddEventHandler('beekersjob:onHijack', function()
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
...
RegisterNetEvent('beekersjob:onCarokit')
AddEventHandler('beekersjob:onCarokit', function()
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
...
RegisterNetEvent('beekersjob:onFixkit')
AddEventHandler('beekersjob:onFixkit', function()
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
...
AddEventHandler('beekersjob:hasEnteredMarker', function(zone)
...
AddEventHandler('beekersjob:hasExitedMarker', function(zone)
	if zone =='VehicleDelivery' then
		NPCTargetDeleterZone = false
	elseif zone == 'Craft' then
		TriggerServerEvent('beekersjob:stopCraft')
		TriggerServerEvent('beekersjob:stopCraft2')
		TriggerServerEvent('beekersjob:stopCraft3')
	elseif zone == 'Garage' then
		TriggerServerEvent('beekersjob:stopHarvest')
		TriggerServerEvent('beekersjob:stopHarvest2')
		TriggerServerEvent('beekersjob:stopHarvest3')
	end
...
AddEventHandler('beekersjob:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()

	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'beekers' and not IsPedInAnyVehicle(playerPed, false) then
...
AddEventHandler('beekersjob:hasExitedEntityZone', function(entity)
...
Citizen.CreateThread(function()
	...
	AddTextComponentSubstringPlayerName(_U('beekers'))
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'beekers' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('beekersjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('beekersjob:hasExitedMarker', LastZone)
			end
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('beekersjob:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity then
				TriggerEvent('beekersjob:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'beekers' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						then
							TriggerServerEvent('service:disableService', 'beekers')
						end
		...
		if IsControlJustReleased(0, 167) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'beekers' then
			OpenMobilebeekersActionsMenu()
		end

		if IsControlJustReleased(0, 178) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'beekers' then
			if NPCOnJob then
...
AddEventHandler('phone:loaded', function(phoneNumber, contacts)
  ...
  TriggerEvent('phone:addSpecialContact', specialContact.name, specialContact.number, specialContact.base64Icon)
end)
