...
local MiseEnFourriere = {coords = vector3(104.47, 6621.96, 31.82)}
...
function MiseEnFourriereFunction()
	local ped = PlayerPedId()
     ...
     if IsPedInAnyVehicle(ped, false) then
          if ESX.PlayerData.job and ESX.PlayerData.job.name == 'beekers' then
               ESX.ShowAdvancedNotification("Beekers", "~b~Destruction du véhicule", "Mise en place du véhicule.", "CHAR_PROPERTY_CAR_MOD_SHOP", 8)
               TaskVehiclePark(ped, vehicle, -196.21, -1324.38, 31.12, 269.46, 0, 20.0, false)
-- Cam
               local camera = CreateCam("DEFAULT_SCRIPTED_CAMERA", 1)
               SetCamCoord(camera, 104.47, 6621.96, 31.82)
...
-- Ouverture de toute les portes
               ...
               ESX.ShowAdvancedNotification("Beekers", "~b~Destruction du véhicule", "Destruction du véhicule ...", "CHAR_PROPERTY_CAR_MOD_SHOP", 8)
               alpha = 255
               ...
               while alpha > 0 do
                    ...
                    Wait(50)
                    if alpha == 0 then
                         TriggerServerEvent("beekers:fourriere")
          ...
          ESX.ShowAdvancedNotification("Beekers", "~b~Destruction du véhicule", "Tu doit etre en véhicule pour faire ça.", "CHAR_PROPERTY_CAR_MOD_SHOP", 8)
     end
end
...
local Interior = GetInteriorAtCoords(104.47, 6621.96, 31.82)
