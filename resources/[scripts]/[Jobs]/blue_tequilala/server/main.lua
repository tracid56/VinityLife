...
RegisterServerEvent('AnnounceTequiOuvert')
AddEventHandler('AnnounceTequiOuvert', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local xPlayers	= ESX.GetPlayers()
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Tequilala', '~b~Annonce Tequilala', 'Le Tequilala vient d\'ouvrir ! Venez faire la fête !', 'CHAR_PROPERTY_BAR_TEQUILALA', 8)
	end
end)

RegisterServerEvent('AnnounceTequiFerme')
AddEventHandler('AnnounceTequiFerme', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local xPlayers	= ESX.GetPlayers()
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Tequilala', '~b~Annonce Tequilala', 'Le Tequilala vient de fermer, passer plus tard !', 'CHAR_PROPERTY_BAR_TEQUILALA', 8)
	end
end)
