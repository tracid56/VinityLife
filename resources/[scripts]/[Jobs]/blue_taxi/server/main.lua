...
AddEventHandler('AnnounceTaxiOuvert', function()
	...
	for i=1, #xPlayers, 1 do
		...
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Taxi', '~b~Annonce Taxi', 'Un chauffeur est disponible ! Envoyez votre position sur le téléphone !', 'CHAR_TAXI', 8)
	end
end)
...
AddEventHandler('AnnounceTaxiFerme', function()
	...
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Taxi', '~b~Annonce Taxi', 'Il n\' y a plus de chauffeurs disponible !', 'CHAR_TAXI', 8)
	end
end)
