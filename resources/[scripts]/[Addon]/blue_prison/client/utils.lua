RegisterCommand("jailmenu", function(source, args)
	...
	if PlayerData.job.name == "police" then
		...
		ESX.ShowNotification("Vous n'êtes pas un agent de police!")
	end
end)
	...
	if #Values["goal"] > 1 then
		...
		function(data, menu)
			...
			if action == "Boiling Broke" or action == "Security" then
				...
				if PlayerData.job.name ~= "police" then
					ESX.ShowNotification("Vous n'avez pas la clé pour aller la-bas!")
					return
				end
			end
...
Citizen.CreateThread(function()
    ...
    AddTextComponentString('Fédérale')
