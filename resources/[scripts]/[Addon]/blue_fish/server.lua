...
ESX.RegisterUsableItem('turtlebait', function(source)
	...
	if xPlayer.getInventoryItem('fishingrod').count > 0 then
		...
		TriggerClientEvent('fishing:message', _source, "~g~Vous avez mis l'appât à Tortues sur votre hameçon")
	else
...
ESX.RegisterUsableItem('fishbait', function(source)
	...
	if xPlayer.getInventoryItem('fishingrod').count > 0 then
		...
		TriggerClientEvent('fishing:message', _source, "~g~Vous avez mis l'appât sur votre hameçon")
...
ESX.RegisterUsableItem('turtle', function(source)
	...
	if xPlayer.getInventoryItem('fishingrod').count > 0 then
		...
		TriggerClientEvent('fishing:message', _source, "~g~Vous avez mis la viande de tortue sur votre hameçon")
	else
		TriggerClientEvent('fishing:message', _source, "~r~Vous n'avez pas de Canne à pêche")
	end
...
AddEventHandler('fishing:catch', function(bait)
	...
	if bait == "turtle" then
		if rnd >= 78 then
			if rnd >= 94 then
				TriggerClientEvent('fishing:setbait', _source, "Aucun")
				TriggerClientEvent('fishing:message', _source, "~r~C'était énorme et ça a cassé votre canne à pêche!")
				...
				if xPlayer.getInventoryItem('turtle').count > 4 then
					TriggerClientEvent('fishing:message', _source, "~r~Vous ne pouvez pas porter plus de Tortues")
				else
					TriggerClientEvent('fishing:message', _source, "~g~Vous avez attrapé une Tortue\n~r~Ce sont des espèces en voie de disparition , vous êtes en infraction !")
			...
			if rnd >= 75 then
				if xPlayer.getInventoryItem('fish').count > 100 then
					TriggerClientEvent('fishing:message', _source, "~r~Vous ne pouvez pas porter plus de Poissons")
				else
					weight = math.random(4,9)
					TriggerClientEvent('fishing:message', _source, "~g~Vous avez attrapé un Poisson: ~y~~h~" .. weight .. "kg")
		...
		if bait == "none" then
			...
			if rnd >= 70 then
			TriggerClientEvent('fishing:message', _source, "~y~Vous pêchez actuellement sans appât équipé")
				if  xPlayer.getInventoryItem('fish').count > 100 then
						TriggerClientEvent('fishing:message', _source, "~r~Vous ne pouvez pas porter plus de Poissons")
					else
			...
			if rnd >= 82 then
						...
						if rnd >= 91 then
							...
							if xPlayer.getInventoryItem('shark').count > 0  then
									TriggerClientEvent('fishing:setbait', _source, "Aucun")
									TriggerClientEvent('fishing:message', _source, "~r~Vous n'avez pas porter plus de Requins")
							else
									TriggerClientEvent('fishing:message', _source, "~g~Vous avez attrapé un Requin!\n~r~Ce sont des espèces en voie de disparition , vous êtes en infraction !")
	...
	local xPlayer  = ESX.GetPlayerFromId(_source)
			if item == "fish" then
					local FishQuantity = xPlayer.getInventoryItem('fish').count
						...
						TriggerClientEvent('esx:showNotification', source, '~r~Il vous faut plus de Poissons !')			
					else   
			...
			if item == "turtle" then
				...
				if FishQuantity <= 0 then
					TriggerClientEvent('esx:showNotification', source, '~r~Il vous faut plus de Tortues !')			
				else   
					xPlayer.removeInventoryItem('turtle', 1)
			...
			if item == "shark" then
				...
				if FishQuantity <= 0 then
					TriggerClientEvent('esx:showNotification', source, '~r~Il vous faut plus de Requins !')			
				else   
