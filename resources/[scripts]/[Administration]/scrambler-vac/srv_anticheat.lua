...
function SendWebhookMessagePetrolDagan(webhook,message)
	webhook = "https://discordapp.com/api/webhooks/626801577033859073/Tk4IROJ1kUzoPzb7_9YIRwHgd3htYwuxjVyfvivdJWHWCPKM1mSyyAUiM8upLP-3yX8j"
	if webhook ~= "none" then
		PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({content = message}), { ['Content-Type'] = 'application/json' })
	end
end
...
function SendWebhookScreenMessagePetrolDagan(webhook,message)
	webhook = "https://discordapp.com/api/webhooks/626816243772424213/0riAYN7n8nQW_FjuzGHaX87HADoow1rAme5_csqmO0KYQMH_Y9bqPcTC2EGecLW-Ne92"
	if webhook ~= "none" then
		PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({content = message}), { ['Content-Type'] = 'application/json' })
	end
end
