...
function openMenu()
	...
	menuTattoo = NativeUI.CreateMenu("Tatouages","Tatouages", 5, 100)
	...
	for _,k in pairs(tattoosCategories) do
		...
		for i,k in pairs(tattoosList[k.value]) do
			...
			sea = NativeUI.CreateItem("Tatouage n°"..i,"")
		...
		qsm.OnItemSelect = function(_, _, index)
			...
			TriggerServerEvent("tattoos:save", currentTattoos, 250, {collection = k.value, texture = index})
