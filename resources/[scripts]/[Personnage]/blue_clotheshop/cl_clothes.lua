...
function ClotheShopAdd(menu)
	...
	local chaussure = _menuPool:AddSubMenu(menu, "Chaussures","",true,true)
	local lunette = _menuPool:AddSubMenu(menu, "Lunettes","",true,true)
	local chapeau = _menuPool:AddSubMenu(menu, "Chapeaux","",true,true)
	local gil = _menuPool:AddSubMenu(menu, "Gillet par balles","",true,true)
	local sac = _menuPool:AddSubMenu(menu, "Sac","",true,true)
	--local montre = _menuPool:AddSubMenu(menu, "Montres","",true,true)
	local chain = _menuPool:AddSubMenu(menu, "Chaines","",true,true)
	local boucle = _menuPool:AddSubMenu(menu, "Boucle d'oreilles","",true,true)
	...
	menu.OnItemSelect = function(_,_,ind)
		if ind == 1 then
			...
			local clotheShop2 = NativeUI.CreateMenu("Magasin", "", 5, 100)
...
local clotheShop = NativeUI.CreateMenu("Magasin", "", 5, 100)
...
function OpenAccessMenus()
    _menuPool:CloseAllMenus()
    mainMenu = NativeUI.CreateMenu("Mes Accessoires", "Accessoires disponibles", 5, 100)
